...
public ResponseEntity<PersonnePhysiqueResponse> soumettre(@PathVariable UUID id, @AuthenticationPrincipal Jwt jwt) {
    ...
    ficheEvenementNotifier.soumission(jwt, request, response, response.getNom() + " " + response.getPrenoms());
    return ResponseEntity.ok(response);
}
...
public ResponseEntity<PersonnePhysiqueResponse> valider(@PathVariable UUID id, @AuthenticationPrincipal Jwt jwt) {
    ...
    ficheEvenementNotifier.validation(jwt, request, response, response.getNom() + " " + response.getPrenoms());
    return ResponseEntity.ok(response);
}
...
public ResponseEntity<PersonnePhysiqueResponse> rejeter(
    ...
    ficheEvenementNotifier.rejet(jwt, request, response, response.getNom() + " " + response.getPrenoms(), motif);
    return ResponseEntity.ok(response);
}
